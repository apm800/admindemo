<template>
    <div class="basicsSupplier">
        <div style="margin: 0 10px 20px 0">
            <el-button  style="margin: 0 20px 0 0" size="mini" type="success" @click="append" v-checkup="':sys:menu:add'">添加供应商</el-button>
            <el-button  style="margin: 0 20px 0 0" size="mini" type="danger" @click="open2" v-checkup="':sys:menu:add'">删除供应商</el-button>
            <el-input style='max-width: 300px;' @keyup.enter.native='requireData(1,PageSize,input21,sorT)' placeholder="请输入内容" v-model="input21" class="input-with-select">
              <el-button slot="append" icon="el-icon-search" @click='requireData(1,PageSize,input21,sorT)'></el-button>
                    ...
                    <el-table
                v-loading="loading1"
                element-loading-text="拼命加载供应商列表..."
            ...
            <el-dialog :title="addType?'修改':'新增'" :visible.sync="addDialog">
                <el-form :model="ruleForm" name="addSupplier" :rules="rules" ref="ruleForm">
                <!--
                    <el-form-item  v-if="addType" style='max-width: 48%;display: inline-block;' label="供应商ID" :label-width="'120px'">
                ...
                -->    
                    <el-form-item  prop="name" style='max-width: 48%;display: inline-block;' label="供应商名称" :label-width="'120px'">
                        <el-input v-model="ruleForm.name" auto-complete="off"></el-input>
                    </el-form-item>
                    <el-form-item  prop="code" style='max-width: 48%;display: inline-block;' label="供应商编码" :label-width="'120px'">
                    ...
                    <el-form-item style='max-width: 48%;display: inline-block;' label="供应商地址" :label-width="'120px'">
                        <el-input v-model="ruleForm.address" auto-complete="off"></el-input>
                    </el-form-item>
                    <el-form-item style='max-width: 48%;display: inline-block;' label="供应商联系人" :label-width="'120px'">
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button  v-if="!addType" @click="resetForm('addSupplier')">重置</el-button>
                    <el-button v-if="!addType" type="primary" @click="submitForm('addSupplier')">确定</el-button>
                    <el-button v-if="addType" type="primary" @click="modiForm('addSupplier')">确定</el-button>
                </div>
            </el-dialog>
    ...
    export default {
        ...
        methods: { 
              ...
              submitForm(formName) {
                ...
                this.axios.post('/dc-admin/admin/partner/save',
                  this.ruleForm,
                  {type: 'payload'}
                  ).then(res =>{
                    console.log(444,res)
              ...
              requireData(page,limit,key,sort){
                  this.loading1 = 1;
                    this.axios.get('/dc-admin/admin/partner/list?page='+page+'&limit='+limit+'&key='+key+'&sidx='+sort.sidx+'&order='+sort.order, {type: 'payload'}
                  ).then(res =>{
              ...
              //批量删除
              open2() {
                this.$confirm('此操作将永久删除选中的供应商, 是否继续?', '提示', {
                  ...
                  if (this.tableChe.length==0) {
                      this.$message({
                        ...
                        this.axios.post('/dc-admin/admin/partner/delete',
                          this.tableChe,
                          {type: 'payload'}
                          ).then(res =>{
                            console.log(444,res)
              ...
              //确认修改
              modiForm(){
                this.axios.post('/dc-admin/admin/partner/update',
                  this.ruleForm,
                  {type: 'payload'}
                  ).then(res =>{
                    console.log(444,res)
