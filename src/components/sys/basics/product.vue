<template>
    <div class="basicsProduct">
        <div style="margin: 0 10px 20px 0">
            <el-button  style="margin: 0 20px 0 0" size="mini" icon='el-icon-refresh' :loading='refreshBtn.now' type="success" @click="refresh" v-checkup="':sys:menu:add'">{{refreshBtn.con}}</el-button>
    ...
    export default {
        ...
        methods: { 
              refresh() {
                ...
                this.axios.post('/dc-admin/admin/product/download',
                  {type: 'payload'}
                  ).then(res =>{
                  	console.log(res)
                    if (res.data.code==0) {
              ...
              format_dataSource(row){
              	try{
              		if(row.dataSource == '1'){
                       row.dataSource = 'EDB'
                      }else if(row.dataSource == '2'){
                       row.dataSource = '8069平台'
                      }
              ...
              requireData(page,limit,key,sort){
                  this.loading1 = 1;
                this.axios.get('/dc-admin/admin/product/list?page='+page+'&limit='+limit+'&key='+key+'&sidx='+sort.sidx+'&order='+sort.order, {type: 'payload'}
                  ).then(res =>{
                    console.log(444,res)
                    try{
