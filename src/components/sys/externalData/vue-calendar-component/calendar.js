export default {
    ...
    */
    ...
    /**
     * 获取当前日期上个月或者下个月，跳转到当前月设置
    */
    nowOtherMonth: function (date) {
        const timeArray = this.dateFormat(date).split('/')
        const year = timeArray[0]
        const month = timeArray[1]
        const day = timeArray[2]
        console.log('今天',timeArray)
        let year2 = year
        let month2=month;
        let day2=day;
        var nowDates = new Date();
        year2=nowDates.getFullYear();
        month2=nowDates.getMonth()+1;
        day2=nowDates.getDate();
        if (month2 < 10) {
            month2 = '0' + month2
        }
        if (day2 < 10) {
        ...
        console.log('dsaads',month2)
        const t2 = month2 + '/' + day2 + '/' + year2
        console.log('2sadsda121',t2)
        return new Date(t2)
    },
    /**
     * 跳转到指定月份
    */
    zhidingOtherMonth: function (date) {
        console.log('我们的',date)
        const timeArray = this.dateFormat(date).split('/')
        const year = timeArray[0]
        const month = timeArray[1]
        const day = timeArray[2]
